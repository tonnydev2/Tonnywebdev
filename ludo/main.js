...
class AIPlayer {
  ...
  async makeMove(pawns, rollValue) {
    ...
    if (bestMove) {
      console.log(`🤖 ${this.color} AI chooses pawn ${bestMove.pawn.id}`);
...
function handlePawnMoveComplete(e) {
  ...
  // Check for captures
  const pawn = findPawnById(color, pawnId);
  if (pawn) {
    const captured = checkForCaptures(pawn);
    if (captured) {
...
function checkForCaptures(movedPawn) {
  if (movedPawn.isInHome || movedPawn.isInHomeStretch || movedPawn.isFinished) return false;
  if (isSafePosition(movedPawn.positionIndex)) return false;
...
export function drawAll() {
  ...
  allDice.forEach(dice => dice.draw());
...
function showWinScreen(winnerColor) {
  ...
  winScreen.innerHTML = `
    <div style="text-align: center; padding: 20px;">
      <h1 style="font-size: 2em; margin-bottom: 10px; color: ${winnerColor};">
...
function gameOver() {
  ...
  gameOverDiv.innerHTML = `
    <div style="text-align: center; padding: 20px;">
      <h1 style="font-size: 2.5em; margin-bottom: 20px; color: gold;">
        🏆 GAME COMPLETE! 🏆
      </h1>
